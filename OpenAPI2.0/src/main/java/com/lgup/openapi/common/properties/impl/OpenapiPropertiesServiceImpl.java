package com.lgup.openapi.common.properties.impl;
...
import com.lgup.openapi.common.properties.OpenapiPropertiesService;

@SuppressWarnings({"rawtypes","unchecked"})
public class OpenapiPropertiesServiceImpl implements OpenapiPropertiesService,
		ApplicationContextAware, InitializingBean, DisposableBean {

	private ExtendedProperties openapiProperties = null;
    ...
    public void refreshPropertyFiles() throws Exception {
        ...
        try {
            ...
            if (OpenapiPropertiesService.LOGGER.isErrorEnabled()) {
            	OpenapiPropertiesService.LOGGER.error("[Properties Service] Fail to refresh file properties");
            	OpenapiPropertiesService.LOGGER.error("[Properties Service] Some property files doesn't exist or there are wrong definitions in property files.");
            }
            throw e;
        }
    }
    ...
    public void afterPropertiesSet() throws Exception {
        try {
            ...
            openapiProperties = new ExtendedProperties();
            ...
            while (it.hasNext()) {
                ...
                if (OpenapiPropertiesService.LOGGER.isDebugEnabled()) {
                	OpenapiPropertiesService.LOGGER.debug("[Properties Service] Property which Key = "+key+", value = "+value+" is defined in this configuration file.");
                }

                if (key == null || key.equals("")) {
                    if (OpenapiPropertiesService.LOGGER.isErrorEnabled())
                    	OpenapiPropertiesService.LOGGER.error("[Properties Service] key is essential attribute in a <element>.");
                    throw new Exception("[Properties Service] key is essential attribute in a <element>.");
                }

                openapiProperties.put(key, value);
            }
    ...
    private void loadPropertyRes(Resource resource, String encoding)
            throws Exception {
        if (OpenapiPropertiesService.LOGGER.isDebugEnabled()) {
        	OpenapiPropertiesService.LOGGER.debug("[Properties Service] Property which Key = "+resource.getFilename()+", value = "+encoding+" is defined in this configuration file.");
        }
        ExtendedProperties openapiProperty = new ExtendedProperties();
        openapiProperties.load(resource.getInputStream(), encoding);
        openapiProperties.combine(openapiProperty);
    }
